...
const detail = (akun)=>{
    const contacts = getDatabase()
    //cek contact
    let sama = contacts.find((element)=> element.nama === akun)
        if(!sama){
            console.log(`akun:${akun} tidak ada`)
            return false
        }
    //tampilin detail
    contacts.forEach((element)=>{
        if(element.nama == akun){
            console.log(`nama:${element.nama} email:${element.email} noHp:${element.noHp}`)
        }
    })
}
module.exports = {inputData, removeAcc, seeList, detail}